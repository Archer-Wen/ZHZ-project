     ...
     public static boolean simulateKeyboardEvent(HWND hwnd, int[][] keyCombination) {  
         ...
         for(int[] keys : keyCombination) {  
             for(int i = 0; i < keys.length; i++) {  
            	 User32Ext.USER32EXT.keybd_event((byte) keys[i], (byte) 0, KEYEVENTF_KEYDOWN, 0); // key down  
             }  
             for(int i = keys.length - 1; i >= 0; i--) {  
            	 User32Ext.USER32EXT.keybd_event((byte) keys[i], (byte) 0, KEYEVENTF_KEYUP, 0); // key up  
             }  
         }  
         return true;  
     }  
             ...
             return execute(new Callable<Boolean>() {  
                 ...
                 public Boolean call() throws Exception {  
                     ...
                     for(char c : content.toCharArray()) {  
                         Thread.sleep(5);  
                         User32Ext.USER32EXT.SendMessage(hwnd, WM_CHAR, (byte) c, 0);  
                     }  
                     return true;  
                 }  
         ...
         try {  
             return execute(new Callable<Boolean>() {  
                 ...
                 public Boolean call() throws Exception {  
                	 //User32Ext.USER32EXT.SwitchToThisWindow(hwnd, true);  
                	 //User32Ext.USER32EXT.SetFocus(hwnd);  
                	 User32Ext.USER32EXT.SendMessage(hwnd, WM_SETTEXT, 0, content);  
                     return true;  
                 }  
     ...
     public static boolean simulateClick(final HWND hwnd) {  
         ...
         try {  
             return execute(new Callable<Boolean>() {  
                 ...
                 public Boolean call() throws Exception {  
                	 User32Ext.USER32EXT.SwitchToThisWindow(hwnd, true);  
                	 User32Ext.USER32EXT.SendMessage(hwnd, BM_CLICK, 0, null);  
                     return true;  
                 }  
